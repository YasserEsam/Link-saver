...
export default function AddEditModal({ isOpen, onClose, initialData, onSave }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const cleanAccounts = formData.accounts.filter(
      acc => acc.email.trim() || acc.password.trim()
    );
    onSave({ ...formData, accounts: cleanAccounts });
    onClose();
  };
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{ padding: '1.25rem', overflowY: 'auto', flex: 1 }}>
          <form id="add-edit-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {/* Note */}
            <div>
              <label style={{ display: 'block', marginBottom: '0.3rem', fontSize: '0.8rem', fontWeight: '500' }}>{t('note')}</label>
              <textarea
                className="glass-input"
                value={formData.note}
